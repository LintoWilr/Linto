...
public class 守护之光 : ISlotResolver
{
    ...
    private static float 获取守护阈值() => PvPSMNSettings.Instance.守护之光血量 / 100f;

    public int Check()
    {
        var 守护队友 = PvPSMNSettings.Instance.守护队友;
        var hpThreshold = 获取守护阈值();
        ...
        if (守护队友)
        {
            ...
            if (PartyHelper.Party[PvPSMNSettings.Instance.守护对象].CurrentHpPercent() <= hpThreshold &&
                ...
                return -91;
            var member = PartyHelper.CastableParty.FirstOrDefault(chara => chara.CurrentHpPercent() <= hpThreshold);
    ...
    {
        var 守护队友 = PvPSMNSettings.Instance.守护队友;
        var 守护播报 = PvPSMNSettings.Instance.守护播报;
        var hpThreshold = 获取守护阈值();
