...
public class PVPTargetHelper
{
    ...
    /// 重置野火状态标志位
    /// </summary>
    public static void ResetWildfireStatus() => _wildfireReleased = false;
    ...
    public static bool Check目标罩子(IBattleChara? target) => TargetSelector.Check目标罩子(target);

    public static bool Check目标免控(IBattleChara? target) => TargetSelector.Check目标免控(target);

    public static List<IBattleChara> Get看着目标的人(Group type, IBattleChara target, float range = 50f, uint? actionId = null) => TargetSelector.Get看着目标的人(type, target, range, actionId);

    public static List<IBattleChara> 获取自身周围6码内地天状态敌对玩家() => TargetSelector.获取自身周围6码内地天状态敌对玩家();
    ...
    public static class TargetSelector
    {
        ...
        public static IBattleChara? Get最合适目标(int 技能距离, uint 技能id)
        {
            ...
            _ = Core.Resolve<MemApiSpell>();
            var 最大距离 = Math.Min(PvPSettings.Instance.自动选中自定义范围, 50);
        ...
        public static IBattleChara? Get最近目标(uint? actionId = null)
        {
            ...
            foreach (var member in GetEnemiesInRange(最大距离))
            {
                ...
                var distance = member.DistanceToPlayer();
                if (distance < 最近距离)
                {
                    最近的目标 = member;
                    最近距离 = distance;
        ...
        public static IBattleChara? Get最远目标(uint? actionId = null)
        {
            ...
            _ = Core.Resolve<MemApiSpell>();
            var 最大距离 = PvPSettings.Instance.自动选中自定义范围;
        ...
        public static IBattleChara? Get野火目标(uint? actionId = null)
        {
            ...
            var 最大距离 = Math.Min(PvPSettings.Instance.自动选中自定义范围, 25); // 野火距离限制更严格
            IBattleChara? 最近的目标 = null;
            float 最近距离 = 最大距离;
        ...
        public static IBattleChara Get多斩Target(int 多斩count, uint? actionId = null)
        {
            ...
            foreach (var enemy in GetEnemiesInRange(25))
            {
                ...
                var totalHpPercentage = enemy.CurrentHpPercent() + (enemy.ShieldPercentage / 100f);
...
#pragma warning disable CS0414 // 保留字段用于未来功能扩展
        private static readonly long 上一次AOE目标搜索时间点;
#pragma warning restore CS0414
        ...
        public static bool Check目标罩子(IBattleChara? target) => target != null && target.HasAura(防御状态);
        ...
        public static bool Check目标无敌(IBattleChara? target) => target != null && (target.HasAura(免疫状态1) || target.HasAura(免疫状态4));
        ...
        public static bool Check目标地天(IBattleChara? target) => target != null && target.HasAura(地天状态);
        ...
        /// 免控BUFF列表
        /// </summary>
        public static List<uint> 免控BUFFList =>
        [
        3054u,//防御
        3248u,//活性
        1320u,//明镜
        ...
        public static bool Check目标免控(IBattleChara? target) => target != null && target.HasAnyAura(免控BUFFList);
        ...
        private static bool FilterCharacter(IBattleChara? unit, Filter filter, uint? actionId = null)
        {
            ...
            {
                Filter.可施法 => true,
                Filter.可攻击 => !Check目标不可攻击(unit),
                Filter.无无敌 => !(unit.HasAura(免疫状态1) || unit.HasAura(免疫状态4)),
                Filter.可控制 => !unit.HasAnyAura(免控BUFFList),
                _ => false,
            };
        }
        ...
        {
            var 全部单位 = TargetMgr.Instance.Units;
            var dict = new Dictionary<uint, IBattleChara>();

            // 遍历所有单位
            foreach (var unit in 全部单位.Values)
            {
                // 过滤不符合条件的单位
        ...
        {
            var key = GetGameObjectIdAsUint(unit.GameObjectId);
        ...
        public static List<IBattleChara> Get看着目标的人(Group type, IBattleChara target, float range = 50f, uint? actionId = null)
        {
            ...
            var 全部单位 = Get全部单位(type, range, Filter.None, 29415u);
            foreach (var 单位 in 全部单位.Values)
            {
                ...
                {
                    var distance = 单位.DistanceToPlayer();
                    if (distance <= range)
                    {
                        re.Add(单位);
                    }
