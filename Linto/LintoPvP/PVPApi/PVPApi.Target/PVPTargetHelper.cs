...
public class PVPTargetHelper
{
    ...
    public static IBattleChara 目标模式(int 距离, uint 技能id)
    {
        ...
        {
            var target1 = Core.Me.GetCurrTarget();
            return target1 ?? Core.Me as IBattleChara;
        }
        ...
        // 根据设置选择目标模式
        IBattleChara? target = PvPSettings.Instance.最合适目标
            ? TargetSelector.Get最合适目标(距离 + PvPSettings.Instance.长臂猿, 技能id)
            : TargetSelector.Get最近目标(技能id);
        return target ?? Core.Me as IBattleChara;
    }
