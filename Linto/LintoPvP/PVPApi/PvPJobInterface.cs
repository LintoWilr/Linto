...
public abstract class BaseJobConfig : PvPJobInterface
{
    ...
    /// 获取当前是否有权限 非通用码可用
    /// </summary>
    public virtual void 权限获取() => PVPHelper.权限获取();
    ...
    public virtual void ConfigureSkillBool(uint SkillID, string SkillName, string description, ref bool variable, int ID) => PVPHelper.技能配置2(SkillID, SkillName, description, ref variable, ID);

    public virtual void ConfigureSkillInt(uint SkillID, string SkillName, string description, ref int value, int step, int quickstep, int id) => PVPHelper.技能配置3(SkillID, SkillName, description, ref value, step, quickstep, id);

    public virtual void ConfigureSkillBoolInt(uint SkillID, string SkillName, string IntDescription, string description, ref bool status, ref int value, int step, int quickstep, int id) => PVPHelper.技能配置4(SkillID, SkillName, IntDescription, description, ref status, ref value, step, quickstep, id);
    public virtual void ConfigureSkillSliderFloat(uint SkillID, string SkillName, string IntDescription, ref float value, float min, float max, int id) => PVPHelper.技能配置5(SkillID, SkillName, IntDescription, ref value, min, max, id);
    public virtual void ConfigureSkilldescription(uint SkillID, string SkillName, string description) => PVPHelper.技能解释(SkillID, SkillName, description);
}
public class 职业配置 : BaseJobConfig
{
    public class 自定义
    {
        private static string GetPartyMemberName(int index)
        {
            ...
            return party[index]?.Name?.TextValue ?? string.Empty;
        }
        ...
        public static void 光阴神()
        {
            ...
            {
                ImGui.Text("优先对象:");
                var targetIndex = PartyHelper.Party.IndexOf(targetMember);
            ...
            else
            {
                ...
                if (ImGui.Button("优先玩家名设定该对象"))
                    PvPBRDSettings.Instance.优先对象 = GetPartyMemberName(PvPBRDSettings.Instance.光阴对象);
        ...
        public static void 守护之光()
        {
            ...
            else
            {
                ...
                if (ImGui.Button("优先玩家名设定该对象"))
                    PvPSMNSettings.Instance.优先对象 = GetPartyMemberName(PvPSMNSettings.Instance.守护对象);
        ...
        public static void 刚玉之心()
        {
            ...
            else
            {
                ...
                if (ImGui.Button("优先玩家名设定该对象"))
                    PvPGNBSettings.Instance.优先对象 = GetPartyMemberName(PvPGNBSettings.Instance.刚玉对象);
