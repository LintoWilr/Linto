...
public class HotkeyData
{
    ...
    private static void DrawActionIcon(Vector2 size, uint actionId, bool useAdjustedIcon = false)
    {
        ...
        if (!Core.Resolve<MemApiIcon>().GetActionTexture(actionId, out var textureWrap, useAdjustedIcon))
            return;
    ...
    private static uint 按状态选择技能(uint auraId, uint activeSkillId, uint normalSkillId) => Core.Me.HasLocalPlayerAura(auraId) ? activeSkillId : normalSkillId;

    private static void 绘制状态切换图标(Vector2 size, uint auraId, uint activeSkillId, uint normalSkillId) => DrawActionIcon(size, 按状态选择技能(auraId, activeSkillId, normalSkillId));

    private static void 绘制状态切换外显(Vector2 size, bool isActive, uint auraId, uint activeSkillId, uint normalSkillId) => SpellHelper.DrawSpellInfo(new Spell(按状态选择技能(auraId, activeSkillId, normalSkillId), Core.Me), size, isActive);

    private static void 释放状态切换自施法(uint auraId, uint activeSkillId, uint normalSkillId)
    {
        var slot = EnsureNextSlot();
        // 这里不做额外判定，严格保持原来“根据状态二选一直接放技能”的行为。
        slot.Add(new Spell(按状态选择技能(auraId, activeSkillId, normalSkillId), Core.Me));
    }
    ...
    public class 蛇LB : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            {
                slot.Add(new Spell(39190u, currentTarget));
                return;
            }
            ...
            if (PvPSettings.Instance.最合适目标)
            {
                if (bestTarget != null)
                {
                    slot.Add(new Spell(39190u, bestTarget));
                }
                return;
            }
            ...
            {
                slot.Add(new Spell(39190u, nearestTarget));
            }
        }
    }
    ...
    public class 机工LB : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            if (PvPMCHSettings.Instance.智能魔弹)
            {
                ...
                {
                    slot.Add(new Spell(29415u, bestTarget));
            ...
            slot.Add(new Spell(29415u, currentTarget));
    ...
    public class 龙神LB : IHotkeyResolver
    {
        ...
        {
            var slot = new Slot();
            var gamePos = Core.Me.GetCurrTarget()?.Position ?? Core.Me.Position;
    ...
    private static Vector3 GetMaxSetPosition(Vector3 origin, Vector3 target, float maxDistance)
    {
        ...
        // 计算平方距离（避免开根号，提升性能）
        var distanceSquared = Vector2.DistanceSquared(originXZ, targetXZ);

        // 如果超出最大距离，计算朝向目标的最大距离位置
        if (distanceSquared > maxDistance * maxDistance)
        {
            // 归一化方向向量（避免零向量）
            var direction = targetXZ - originXZ;
    ...
    public class 霰弹枪 : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            if (target != null)
                slot.Add(new Spell(29404u, target));
        }
    }
    public class 画家LB : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            {
                slot.Add(new Spell(39215u, Core.Me));
            }
        }
    ...
    public class 绝枪LB : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            {
                slot.Add(new Spell(29130u, Core.Me));
            }
        }
    }
        ...
        public void Run()
        {
            ...
            if (!PVPHelper.CanActive() && Core.Me.InCombat() && Core.Me.LimitBreakCurrentValue() > 3500)
            {
                if (PvPSAMSettings.Instance.多斩模式)
                {
                    ...
                    slot.Add(new Spell(29537u, target));
    ...
    public class 诗人LB : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            if (Core.Me.InCombat() && Core.Me.LimitBreakCurrentValue() >= 4000)
                slot.Add(new Spell(29401, Core.Me));
        }
    }
    public class 抗死 : IHotkeyResolver
    {
        ...
        public void Run() => 释放状态切换自施法(3245u, 29697u, 29698u);
    }
    public class 黑白魔元切换 : IHotkeyResolver
    {
        ...
        public void Run() => 释放状态切换自施法(3245u, 29702u, 29703u);
    }
        ...
        public void Run()
        {
            ...
            if (Core.Me.InCombat() & Core.Me.LimitBreakCurrentValue() >= 3000)
                if (PvPRDMSettings.Instance.南天自己)
                    slot.Add(new Spell(41498u, Core.Me));
                else
                {
                    var currentTarget = Core.Me.GetCurrTarget();
                    if (currentTarget == null)
                    ...
                    slot.Add(new Spell(41498u, currentTarget));
                }
        }
    }
    public class 后射 : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            var target = Core.Me.GetCurrTarget();
            if (target != null)
                slot.Add(new Spell(29399, target));
        }
    }
    ...
    public class 以太步 : IHotkeyResolver
    {
        ...
        public void Run()
        {
            ...
            var target = Core.Me.GetCurrTarget();
            if (target != null)
                slot.Add(new Spell(29660u, target));
        }
    }
