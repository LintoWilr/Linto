...
public class 瞬发连击 : ISlotResolver
{
    public SlotMode SlotMode { get; }
    public int Check()
    {
        if (!PvPPCTOverlay.PCTQt.GetQt("瞬发连击"))
        {
            return -1;
        }
    ...
    public void Build(Slot slot)
    {
        ...
        {
            PVPHelper.通用技能释放(slot, PCTSkillID.技能连击3流水之蓝, 25);

        }
        //瞬发2疾风
        else if ((Core.Me.HasLocalPlayerAura(PCTSkillID.buff以太色调)))
        {
            PVPHelper.通用技能释放(slot, PCTSkillID.技能连击2疾风之绿, 25);
        }
        //瞬发1火炎
        else
        {
            PVPHelper.通用技能释放(slot, PCTSkillID.技能连击1火炎之红, 25);
        }
    }
}