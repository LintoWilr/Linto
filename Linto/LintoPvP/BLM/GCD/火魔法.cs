...
public class 核爆 : ISlotResolver
{
    ...
    public int Check()
    {
        ...
        if (PvPBLMSettings.Instance.最佳AOE)
        {
            ...
            var target = TargetHelper.GetMostCanTargetObjects(29651u, PvPBLMSettings.Instance.最佳人数);
            if (target == null)
            {
                return -99;
            }
            if (Core.Me.Distance(target) > 25 + PvPSettings.Instance.长臂猿)
            {
                return -5;
            }
            {
