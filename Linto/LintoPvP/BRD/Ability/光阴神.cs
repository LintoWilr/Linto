...
namespace Linto.LintoPvP.BRD.Ability
{
    public class 光阴神 : ISlotResolver
    {
        ...
        public void Build(Slot slot)
        {
            ...
            if (光阴队友)
            {
                if (PartyHelper.Party[PvPBRDSettings.Instance.光阴对象].HasCanDispel()&&PartyHelper.CastableParty.Contains(PartyHelper.Party[PvPBRDSettings.Instance.光阴对象]))
                    target = PartyHelper.Party[PvPBRDSettings.Instance.光阴对象];
                else target = PartyHelper.CastableParty.FirstOrDefault(chara => chara.HasCanDispel());
                if (target == null)
                {
                    return;
                }
