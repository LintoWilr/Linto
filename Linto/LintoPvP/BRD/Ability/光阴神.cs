...
namespace Linto.LintoPvP.BRD.Ability
{
    public class 光阴神 : ISlotResolver
    {
        ...
        private static bool 光阴对象索引有效(int index) => index >= 0 && index < PartyHelper.Party.Count;

        public int Check()
        {
            var 光阴队友 = PvPBRDSettings.Instance.光阴队友;
            ...
            if (光阴队友)
            {
                ...
                var member = PartyHelper.CastableParty.FirstOrDefault(chara => chara.HasCanDispel());
        ...
        {
            var 光阴队友 = PvPBRDSettings.Instance.光阴队友;
            var 光阴播报 = PvPBRDSettings.Instance.光阴播报;
            IBattleChara? target;
